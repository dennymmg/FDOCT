...
* ] key increases thresholding by 10 dB in final Bscan
* [ key decreases thresholding by 10 dB in final Bscan
* s key saves the current bscan to disk
*
...
int main()
{
	...
	bool expchanged = false, accummode = false, refreshkeypressed = false, skeypressed = false;
	int n;
	...
	uint  indextemp, indextempJ, indextempS, averagescount = 1, i = 0;
		...
		while (1)	//camera frames acquisition loop, which is inside the try
		{
			...
			while(indextempJ < averagescount)
			{
				...
				if(ret == 1)
				{
					...
					indextempJ++; 
					cout << i <<":  " <<"indextempJ = " << indextempJ << endl;
...
inline void printMinMaxAscan(Mat bscandb, uint ascanat, int numdisplaypoints, Mat& statusimg)
{
	...
	sprintf(textbuffer, "Max of Ascan at %d = %3.1f dB", ascanat, maxVal);
	thirdrowofstatusimg = Mat::zeros(cv::Size(600, 50), CV_64F);
	putText(statusimg, textbuffer, Point(0, 180), FONT_HERSHEY_SIMPLEX, 1, Scalar(255, 255, 255), 3, 1);
	sprintf(textbuffer, "Min of Ascan at %d = %3.1f dB", ascanat, minVal);
